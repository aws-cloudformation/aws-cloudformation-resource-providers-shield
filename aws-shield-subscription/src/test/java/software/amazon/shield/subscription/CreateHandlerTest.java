    ...
    public void handleRequest_SimpleSuccess() {
        final ResourceHandlerRequest<ResourceModel> request =
                ResourceHandlerRequest.<ResourceModel>builder()
                        ...
                        .nextToken(SubscriptionTestData.NEXT_TOKEN)
                        .build();

        final CreateSubscriptionResponse createSubscriptionResponse = CreateSubscriptionResponse.builder().build();
        final DescribeSubscriptionResponse describeSubscriptionResponse =
                SubscriptionTestData.DESCRIBE_SUBSCRIPTION_RESPONSE;

        doReturn(createSubscriptionResponse)
                .when(this.proxy).injectCredentialsAndInvokeV2(any(CreateSubscriptionRequest.class), any());
